  ...
  return (
    <main className="min-h-screen bg-[#0f1117] text-white">
      ...
      <nav className="border-b border-white/5 px-8 py-4 flex items-center justify-between max-w-7xl mx-auto">
        <span className="font-semibold text-sm tracking-wide text-white">ParkSim</span>
        <Link
          href="/simulation"
      ...
      <section className="max-w-4xl mx-auto px-8 pt-28 pb-20">
        ...
        <p className="text-slate-400 text-base leading-relaxed max-w-xl mb-10">
          Model komputasional yang mensimulasikan bagaimana keberadaan juru parkir liar
          mempengaruhi keputusan belanja pelanggan — dan seberapa besar revenue yang
          hilang seiring waktu akibat efek memori dan penyebaran informasi.
        </p>
        ...
        <div className="flex items-center gap-4">
          ...
          <span className="text-slate-600 text-sm">Sesuaikan parameter, lihat hasilnya secara real-time</span>
        </div>
      </section>
      ...
      <section className="max-w-4xl mx-auto px-8 py-16">
        ...
        <div className="grid grid-cols-4 gap-4">
          {[
            { step: '01', title: 'Atur Parameter', desc: 'Biaya parkir, jumlah agen, radius pasar, bobot skor, dan lainnya.' },
            { step: '02', title: 'Jalankan ABM', desc: 'Setiap agen membuat keputusan independen berdasarkan utilitas yang dihitung.' },
            { step: '03', title: 'Lihat Dinamika 3D', desc: 'Pantau pergerakan agen dan persebaran WOM di scene interaktif.' },
            { step: '04', title: 'Analisis Dampak', desc: 'Bandingkan revenue, kunjungan, dan market share antar skenario.' },
          ].map((item) => (
            <div key={item.step} className="space-y-2">
              <div className="text-[10px] font-mono text-slate-600">{item.step}</div>
